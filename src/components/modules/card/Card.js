import React from 'react';
import styles from './Card.module.css';

const BlogCard = () => {
  return (
    <div className="col-md-6 col-lg-4 px-lg-4">
      {/* Card */}
      <div
        className={`card card-full border-0 blog-two mb-5 rounded-3 overflow-hidden ${styles.blogCard}`}
        data-aos="fade-up"
      >
        <div className="blog-two-img">
          <a href="#">
            <img
              className="img-fluid"
              src="/images/Food.jpg"
              alt="alt title"
            />
          </a>
        </div>
        <div className="card-body px-0">
          <div className="card-title">
            <h3 className={`h5 mb-3 ${styles.blogTitle}`}>
              <a href="#" className={styles.blogTitleLink}>عنوان مطلب وبلاگ</a>
            </h3>
            <div className={`meta ${styles.blogMeta}`}>
              <svg
                className={`bi bi-egg-fried me-2 ${styles.foodIcon}`}
                width=".8rem"
                height=".8rem"
                viewBox="0 0 16 16"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  fillRule="evenodd"
                  d="M13.665 6.113a1 1 0 0 1-.667-.977L13 5a4 4 0 0 0-6.483-3.136 1 1 0 0 1-.8.2 4 4 0 0 0-3.693 6.13 1 1 0 0 1 .2 1 4 4 0 0 0 6.67 4.087 1 1 0 0 1 1.262-.152 2.5 2.5 0 0 0 3.715-2.905 1 1 0 0 1 .341-1.113 2.001 2.001 0 0 0-.547-3.478zM14 5c0 .057 0 .113-.003.17a3.001 3.001 0 0 1 .822 5.216 3.5 3.5 0 0 1-5.201 4.065 5 5 0 0 1-8.336-5.109A5 5 0 0 1 5.896 1.08 5 5 0 0 1 14 5z"
                />
                <circle cx="8" cy="8" r="3" />
              </svg>
              <span>4 اردیبهشت 1403</span>
            </div>
          </div>
          <p className={`mb-0 ${styles.blogContent}`}>
            لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم از صنعت چاپ.
          </p>
        </div>
      </div>
      {/* End card */}
    </div>
  );
};

export default BlogCard;