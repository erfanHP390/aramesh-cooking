(function () {
    ...
    if (typeof Typed !== "undefined" && x) {
      [].forEach.call(x, function (element) {
        (function (el) {
          ...
          try {
            // Check if typo exists and is valid JSON
            var parsedOptions = typo ? JSON.parse(typo) : {};
  ...
  const myProgress = function () {
    ...
    function myFunction() {
      ...
      document
        .querySelector(".progress-one, .progress-two, .progress-three")
        .setAttribute("value", scrolled);
    }
  ...
  const sub_dropdown_js = function () {
    // submenu
    const yaghout_submenu = function yaghout_submenu() {
      var yaghout_toggle = document.querySelectorAll(
        ".dropdown-menu a.dropdown-toggle"
      );
      var _loop = function _loop(i) {
        yaghout_toggle[i].addEventListener("click", function (event) {
          event.stopPropagation();
          event.preventDefault();

          yaghout_toggle[i].nextElementSibling.classList.toggle("show");
          yaghout_toggle[i].parentNode.classList.toggle("show");
        });
        window.addEventListener("mouseup", function (event) {
          if (
            event.target != yaghout_toggle[i].nextElementSibling &&
            event.target.parentNode != yaghout_toggle[i].nextElementSibling &&
            event.target.classList.contains("dropdown-toggle") != true
          ) {
            yaghout_toggle[i].nextElementSibling.classList.remove("show");
            yaghout_toggle[i].parentNode.classList.remove("show");
          }
        });
      };

      for (var i = 0; i < yaghout_toggle.length; i++) {
        _loop(i);
      }
    };
    ...
    const close_submenu = function close_submenu() {
      var dropdown_a = document.querySelectorAll(
        ".dropdown > .dropdown-menu > .dropdown-submenu > .dropdown-toggle"
      );
      var dropdownMenu_a = document.querySelectorAll(
        ".dropdown > .dropdown-menu > .dropdown-submenu > .submenu"
      );
